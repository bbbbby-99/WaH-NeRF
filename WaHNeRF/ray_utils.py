...
def resample_along_rays(origins, directions, radii, t_vals, weights, randomized, stop_grad, resample_padding, ray_shape, perturb_pts=None, perturb_zs=None):
    """Resampling.
    ...
    if perturb_zs is not None:
        new_t_vals = new_t_vals + perturb_zs
    means, covs = cast_rays(new_t_vals, origins, directions, radii, ray_shape)
    if perturb_pts is not None:
        means = means + perturb_pts
    return new_t_vals, (means, covs)


def volumetric_rendering(rgb, density, t_vals, dirs, white_bkgd, perturb=None, offset=None):
