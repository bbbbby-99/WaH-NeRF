...
def subsample_patches(images, images_pertube, pixles, patch_size, batch_size, batching='all_images', Flag = True, Flag2 = True):
  """Subsamples patches."""
  ...
  images = images[scale]
  if not Flag2:
    images_pertube = images_pertube[scale]
  ...
  if Flag2:
      if isinstance(images, np.ndarray):
          ...
          fn = lambda x: x[idx_img, patch_idx[Ellipsis, 1], patch_idx[Ellipsis, 0]]
          images = namedtuple_map(lambda r: fn(r), images)
          out = images
  else:
      if isinstance(images, np.ndarray):
          out = images[idx_img, patch_idx[Ellipsis, 1], patch_idx[Ellipsis, 0]].reshape(-1, 3)
          out_pertube = images_pertube[idx_img, patch_idx[Ellipsis, 1], patch_idx[Ellipsis, 0]].reshape(-1, 3)
      else:
          fn = lambda x: x[idx_img, patch_idx[Ellipsis, 1], patch_idx[Ellipsis, 0]]
          images = namedtuple_map(lambda r: fn(r), images)
          out = images
          images_pertube = namedtuple_map(lambda r: fn(r), images_pertube)
          out_pertube = images_pertube
  if Flag:
    pixles = pixles[idx_img, patch_idx[Ellipsis, 1], patch_idx[Ellipsis, 0]]
    return out, pixles, np.ones((n_patches, 1), dtype=np.float32) * scale
  else:
      if Flag2:
        return out, np.ones((n_patches, 1), dtype=np.float32) * scale
      else:
        return out, out_pertube, np.ones((n_patches, 1), dtype=np.float32) * scale

def get_dataset(dataset_name, base_dir, split, factor=4, device=torch.device("cpu"), pertube = False):
    d = dataset_dict[dataset_name](base_dir, split, factor=factor, device=device, pertube = pertube)
    return d


def get_dataloader(dataset_name, base_dir, split, factor=4, batch_size=None, shuffle=True, device=torch.device("cpu"), pertube = False):
    d = get_dataset(dataset_name, base_dir, split, factor, device, pertube)
...
class NeRFDataset(Dataset):
    def __init__(self, base_dir, split, spherify=True, near=2, far=6, white_bkgd=False, factor=1, n_poses = 500,
                 radius=None, radii=None, h=None, w=None, device=torch.device("cpu"), pertube=False):
    ...
    def generate_spherical_poses(self, n_poses=500):
        if not self.pertube:
            self.poses = generate_spherical_cam_to_world(self.radius, n_poses)
            self.cam_to_world = self.poses[:, :3, :4]
        else:
            self.poses, self.poses_pertube = generate_spherical_cam_to_world(self.radius, n_poses, self.pertube)
            self.cam_to_world = self.poses[:, :3, :4]
            self.cam_to_world_pertube = self.poses_pertube[:, :3, :4]

    def generate_spiral_poses(self, n_pose=500):
    ...
    def generate_rays(self):
        ...
        self.rays = Rays(
            ...
            far=ones * self.far)
        if  self.split == "render":
            self.rays_pertube = None
            self.rays, self.rays_scale = (
                subsample_patches([self.rays],self.rays_pertube, self.images, patch_size=8, batch_size=25600000, batching='all_images', Flag=False))
        elif self.split == "train":
            self.rays_pertube = None
            self.rays, self.images, self.rays_scale = (
                subsample_patches([self.rays], self.rays_pertube, self.images, patch_size=8, batch_size=25600000, batching='all_images'))

    def generate_rays_pertube(self):
        """Computes rays using a General Pinhole Camera Model
        Assumes self.h, self.w, self.focal, and self.cam_to_world exist
        """
        x, y = np.meshgrid(
        ...
        directions = ((camera_directions[None, ..., None, :] * self.cam_to_world[:, None, None, :3, :3]).sum(
            axis=-1))  # Translate camera frame's origin to the world frame
        directions_pertube = ((camera_directions[None, ..., None, :] * self.cam_to_world_pertube[:, None, None, :3, :3]).sum(
            axis=-1))
        origins = np.broadcast_to(self.cam_to_world[:, None, None, :3, -1], directions.shape)
        origins_pertube = np.broadcast_to(self.cam_to_world_pertube[:, None, None, :3, -1], directions.shape)
        viewdirs = directions / np.linalg.norm(directions, axis=-1, keepdims=True)
        viewdirs_pertube = directions_pertube / np.linalg.norm(directions_pertube, axis=-1, keepdims=True)
        # Distance from each unit-norm direction vector to its x-axis neighbor
        dx_pertube = np.sqrt(np.sum((directions_pertube[:, :-1, :, :] - directions_pertube[:, 1:, :, :]) ** 2, -1))
        dx = np.sqrt(np.sum((directions[:, :-1, :, :] - directions[:, 1:, :, :]) ** 2, -1))
        dx = np.concatenate([dx, dx[:, -2:-1, :]], 1)
        dx_pertube = np.concatenate([dx_pertube, dx_pertube[:, -2:-1, :]], 1)
        # Cut the distance in half, and then round it out so that it's
        # halfway between inscribed by / circumscribed about the pixel.
        radii = dx[..., None] * 2 / np.sqrt(12)
        radii_pertube = dx_pertube[..., None] * 2 / np.sqrt(12)
        ones = np.ones_like(origins[..., :1])
        ones_pertube = np.ones_like(origins_pertube[..., :1])
        ...
        self.rays_pertube = Rays(
            origins=origins_pertube,
            directions=directions_pertube,
            viewdirs=viewdirs_pertube,
            radii=radii_pertube,
            lossmult=ones_pertube,
            near=ones * self.near,
            far=ones * self.far)
        if  self.split == "render":
            if not self.pertube:
                ...
                self.rays, self.rays_pertube, self.rays_scale = (
                    subsample_patches([self.rays],[self.rays_pertube], self.images, patch_size=8, batch_size=25600000, batching='all_images', Flag=False, Flag2=False))
        elif self.split == "train":
            self.rays_pertube = None
            self.rays, self.images, self.rays_scale = (
                subsample_patches([self.rays],self.rays_pertube, self.images, patch_size=8, batch_size=25600000, batching='all_images'))
    ...
    def ray_to_device(self, rays):
        return namedtuple_map(lambda r: torch.tensor(r).float().to(self.device), rays)

    def __getitem__(self, i):
        if self.split == "render":
            if not self.pertube:
                ray = namedtuple_map(lambda r: r[i], self.rays)
                return self.ray_to_device(ray)  # Don't put on device, will batch it using config.chunks in mipNeRF.render_image() function
            else:
                ray = namedtuple_map(lambda r: r[i], self.rays)
                rays_pertube = namedtuple_map(lambda r: r[i], self.rays_pertube)
                return self.ray_to_device(ray), self.ray_to_device(rays_pertube)
        else:
            ray = namedtuple_map(lambda r: r[i], self.rays)
...
class Multicam(NeRFDataset):
    ...
    def __init__(self, base_dir, split, factor=1, spherify=True, white_bkgd=True, near=2, far=6, radius=4, radii=1,
        ...
        super(Multicam, self).__init__(base_dir, split, factor=factor, spherify=spherify, near=near, far=far,
...
class Blender(NeRFDataset):
    ...
    def __init__(self, base_dir, split, factor=1, spherify=False, white_bkgd=True, near=2, far=6, radius=4, radii=1,
                 h=800, w=800, device=torch.device("cpu"), pertube=False):
        super(Blender, self).__init__(base_dir, split, factor=factor, spherify=spherify, near=near, far=far,
                                      white_bkgd=white_bkgd, radius=radius, radii=radii, h=h, w=w, device=device, pertube=pertube)
        ...
        if self.split == 'train':
            for i in range(len(meta['frames'])):
                ...
                if i in [20, 50, 90]:
                    ...
                    images.append(image)
        if self.split == 'test':
            for i in range(len(meta['frames']))[::8]:
                frame = meta['frames'][i]
                fname = os.path.join(self.base_dir, frame['file_path'] + '.png')
                with open(fname, 'rb') as imgin:
                    image = np.array(Image.open(imgin), dtype=np.float32) / 255.
...
class LLFF(NeRFDataset):
    def __init__(self, base_dir, split, factor=4, spherify=False, near=0, far=1, white_bkgd=False,
        ...
        super(LLFF, self).__init__(base_dir, split, spherify=spherify, near=near, far=far, white_bkgd=white_bkgd,
    ...
    def generate_spiral_poses(self, n_poses=500):
